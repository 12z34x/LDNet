...
class ST(Dataset):
    def __init__(self, opt, mode):
        ...
        if self.mode == "train":
            self.transform = transforms.Compose([
                dtf.FixedNoMaskResize(size=opt.input_size),#不调用config,A
                # dtf.RandomFilter(),
            ...
            self.transform = transforms.Compose([
                dtf.FixedNoMaskResize(size=opt.input_size),  # A
                # dtf.RandomFilter(),
                dtf.Normalize(**opt.norm_cfg),
                dtf.ToTensor()])
    ...
    def __getitem__(self, index):
        ...
        label_path = osp.join(self.labels_dir, dir_name+'_'+img_name[:-4]+'.hdf5')#get label s.hdf5 A 
