...
import random
from dataloaders.datasets import drones
...
def make_data_loader(opt, mode="train"):
    ...
    dataset = drones.DroneCC(opt, mode)
...
class Trainer(object):
    def __init__(self, mode):
        ...
        self.scheduler = optim.lr_scheduler.MultiStepLR(
            self.optimizer,
            milestones=[4,8,16,32,64,128,256,512,1024,2048],#round(opt.epochs * x) for x in opt.steps],
            gamma=opt.gamma)
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            ...
            # opt.norm_cfg["para"]=random.randint(10,60)
