...
#         return out
#*******************SANet*********************
class CSMNets(nn.Module):
    def __init__(self, opt):
        super(CSMNets, self).__init__()
        #***********SANet***************
        in_channels = 512
        use_bn=True
        ...
        self.atrous_conv = nn.Conv2d(part_channels, part_channels*2, kernel_size=3,
                                stride=1, padding=2, dilation=2, bias=False)#dilation 与padding相同即可
    ...
    def forward(self, input):
        ...
        mid_level_feat = self.aspp(mid_level_feat)#64 channels
        mid_level_feat=self.atrous_conv(mid_level_feat)
        #x1=self.atrous_conv(x1)
        # high2_level_feat= self.atrous_conv(x1)#self.rfb(x1)#128 channels
        ...
        #region = self.region(high1_level_feat)#high1_level_feat)
        density = self.density(mid_level_feat)#high2_level_feat)
