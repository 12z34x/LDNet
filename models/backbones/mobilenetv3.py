...
class MobileNetV3_Large(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.bneck = nn.Sequential(
            ...
            Block(5, 40, 120, 40, nn.ReLU(inplace=True), SeModule(40), 1, 1),
            Block(3, 40, 240, 80, hswish(), None, 1, 1),# 1
        ...
        self.high_level_features = self.bneck[7:13]
...
class MobileNetV3_Small(nn.Module):
    ...
    def forward(self, x):
        outer = self.hs1(self.bn1(self.conv1(x)))
        low_level_feat = self.low_level_features(outer)
        out = self.high_level_features(low_level_feat)
        return out, low_level_feat
