...
def show_imager(num,img, coordinate):
    ...
    plt.savefig('/home/twsf/data/ST_CC/part_A/augment/'+str(num)+'.jpg')
    #plt.show()
...
if __name__ == '__main__':
    dataset = DroneCC('/home/twsf/data/ST_CC/part_A/', 'train')#only process train B
    ...
    for i in range(0,int(longth)):#use double circulation
        ...
        # semi_h=int(sample["height"]/down_size)*down_size
        # semi_w=int(sample["width"]/down_size)*down_size
        # height=2*semi_h
        # width=2*semi_w
        img=cv2.resize(img,(width*2,height*2))#width and height
        ...
        for j in (img1,img2,img3,img4):
            ...
            cv2.imwrite('/home/twsf/data/ST_CC/part_A/augment/'+aug_f+'/'+aug_i.split('.')[0]+'-'+str(counter)+'.jpg',j)
            # print('/home/twsf/data/ST_CC/part_A/augment/'+aug_f+'/'+aug_i.split('.')[0]+'-'+str(counter)+'.jpg')
